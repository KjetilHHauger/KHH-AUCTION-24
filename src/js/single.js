  ...
  const slides = media
    .map(
      ...
      <div class="carousel-slide ${index === 0 ? 'block' : 'hidden'}">
        <img src="${img.url}" alt="${img.alt}" class="w-full h-64 object-contain rounded-lg mx-auto font-body">
      </div>`
    )
    .join('');
...
async function loadAuctionDetails() {
  ...
  ${token ? `
    ...
    </div>` : '<p class="text-red-500 text-xl font-bold mt-4 text-center font-header">Login to place a bid.</p>'}
  ${bidsOverview}
`;
