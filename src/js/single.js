...
async function placeBid() {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}auction/listings/${auctionId}/bids`,
      {
        method: 'POST',
        headers: {
          'X-Noroff-API-Key': '04cc0fef-f540-4ae1-8c81-5706316265d4',
...
function renderCarousel(media) {
  if (!media || media.length === 0) {
    return '<p class="text-gray-500 font-body">No images available.</p>';
  }
  ...
  const slides = media
    .map(
      ...
      <div class="carousel-slide ${index === 0 ? 'block' : 'hidden'}">
        <img src="${img.url}" alt="${
  img.alt
}" class="w-full h-64 object-contain rounded-lg mx-auto font-body">
      </div>`,
    )
    .join('');
...
function renderBids(bids) {
  if (!bids || bids.length === 0) {
    return '<p class="text-gray-500 font-body">No bids placed yet.</p>';
  }
  ...
  const bidsList = bids
    ...
    .map(
      ...
      <div class="flex justify-between p-2 border-b">
        <span class="font-body">${bid.bidder.name}</span>
        <span class="font-semibold text-primary font-body">$${bid.amount}</span>
      </div>`,
    )
    .join('');
...
async function loadAuctionDetails() {
  ...
  <h1 class="text-3xl font-bold text-gray-800 text-center mb-6 overflow-hidden font-header">${title}</h1>
  ...
  highestBid || 'No bids yet'
      ...
      <p id="timeLeft" class="text-lg font-medium text-tertiary text-center font-body">Time Left: <span class="text-prussian-blue">${timeLeft}</span></p>
    </div>
  </div>
  ${
  token
    ? `
    <div class="mt-6 w-full font-body">
      <label for="bidAmount" class="block text-sm font-medium text-gray-700 font-header">Place a Bid</label>
      <div class="flex flex-col space-y-4 mt-2">
        <input type="number" id="bidAmount" placeholder="Enter bid amount" class="p-2 border rounded-lg flex-grow font-body">
        <button onclick="placeBid()" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-prussian-blue font-header">
          Place Bid
        </button>
      </div>
    </div>`
    : '<p class="text-red-500 text-xl font-bold mt-4 text-center font-header">Login to place a bid.</p>'
}
  ${bidsOverview}
`;
...
function setupCountdown(endsAt) {
  ...
  const interval = setInterval(() => {
    ...
    if (timeLeftElement) {
      timeLeftElement.innerHTML = `Time Left: <span class="text-prussian-blue font-body">${timeLeft}</span>`;
    }
  }, 1000);
}
